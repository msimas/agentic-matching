...
module already has in hand (fndds_texts/off_texts), consistent with this being a
...
def _term_frequency(term: str, fndds_texts: list[Any], off_texts: list[Any]) -> dict[str, Any]:
    term_lower = term.lower()
    fndds_hits = sum(1 for t in fndds_texts if isinstance(t, str) and term_lower in t.lower())
    off_hits = sum(1 for t in off_texts if isinstance(t, str) and term_lower in t.lower())
    return {
        "kind": "term_frequency",
        "term": term,
        ...
        "off_count": off_hits,
        "off_fraction": round(off_hits / len(off_texts), 4) if off_texts else 0.0,
    }


def _sample_records(term: str, side: str, fndds_texts: list[Any], off_texts: list[Any]) -> dict[str, Any]:
    side = side if side in ("fndds", "off") else "fndds"
    texts = fndds_texts if side == "fndds" else off_texts
    term_lower = term.lower()
...
def fulfill_requests(requested: list[dict[str, Any]], fndds_texts: list[Any], off_texts: list[Any]) -> list[dict[str, Any]]:
    ...
    for req in (requested or [])[:MAX_REQUESTS]:
        ...
        if kind == "term_frequency" and term:
            answers.append(_term_frequency(term, fndds_texts, off_texts))
        elif kind == "sample_records" and term:
            answers.append(_sample_records(term, req.get("side", "fndds"), fndds_texts, off_texts))
        else:
