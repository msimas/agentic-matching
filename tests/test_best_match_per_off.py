...
from agentic_matching.linking.evaluate import best_match_per_off
...
def test_keeps_only_highest_probability_candidate_per_off_record():
    out = best_match_per_off(_predictions())
...
def test_min_probability_filters_before_dedup():
    out = best_match_per_off(_predictions(), min_probability=0.5)
...
def test_empty_predictions_returns_empty():
    ...
    out = best_match_per_off(empty)
    assert out.empty


def test_all_rows_below_threshold_returns_empty():
    out = best_match_per_off(_predictions(), min_probability=0.99)
    assert out.empty
